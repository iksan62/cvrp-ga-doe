...
Individual initializeIndividualGGA(
    int attemptgenerate,
    int numCities,
    int numVehicle,
    int vehicleCapacity,
    ...
    std::vector<std::tuple<int, int, int, int, int, double>>& routeDetail)
{
    ...
    int iterationInitialPopulation = 0, maxIterationInitialPopulation = 10;
    int repairingIteration = 0, maxRepairingIteration = 3 * numVehicle;
    int servedCount = 0;
    attemptgenerate = 0;
    do {
        ...
        do {
            ...
            for (int v = 0; v < numVehicle; ++v) {
                routeNum = v;
                startNode = j;
                individual.route[j] = 0; servedCities[0] = 1; j = j + 1;
                individual.route[j] = fixedNode[v];
                currentLoad = demand[fixedNode[v]].second;
                travelDistance = distanceMatrix[individual.route[j - 1]][individual.route[j]];
                j = j + 1;
                ...
                if (allCitiesServed(servedCities, numCities))
                {
                    ...
                    do {
                        ...
                        individual.route[j] = randomNumber;
                        servedCities[randomNumber] = 1;
                        travelDistance = travelDistance + distanceMatrix[individual.route[j - 1]][individual.route[j]];
        ...
        if (numCities != servedCount) {
            do {
                int j = 0; int node;
                int routeNum = 0, feasibility = 0, startNode = 0, endNode = 0, currentLoad = 0, travelDistance = 0;
                std::fill(routeDetailTemporary.begin(), routeDetailTemporary.end(), std::make_tuple(0, 0, 0, 0, 0, 0));
                int routeNumTemporary = 0, feasibilityTemporary = 0, startNodeTemporary = 0, endNodeTemporary = 0, currentLoadTemporary = 0, travelDistanceTemporary = 0;
                ...
                for (int v = 0; v < numVehicle; ++v) {
                    ...
                    if (feasibility == 1) {
                        ...
                        do
                        {
                            temporaryRouteChecker[j] = individual.route[startNode];
                            ++j;
                            ++startNode;
                            node = individual.route[startNode];
                ...
                std::copy(temporaryRouteChecker.begin(), temporaryRouteChecker.end(), individual.route.begin());
    ...
    const std::vector<std::vector<double>>& distanceMatrix)
{
    ...
    for (igenerate = 0; igenerate < populationSize; ++igenerate) {
        ...
        else if (igenerate < populationSize / 5 * 4)
        {
            ...
            population[igenerate] = initializeIndividualGGA(attemptgenerate, numCities, numVehicle, vehicleCapacity, demand, coordinates, benchmarkID, distanceMatrix, routeDetail);
            servedCount = countServedCities(servedCities);
            int numVisited = countVisitedCities(population[igenerate]);
        }
...
int main() {
    ...
    std::string path2 = std::string("d:\\FoG(HNRI)=")
        + benchmarkID
