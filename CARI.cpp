...
std::vector<Individual> initializePopulation(int populationSize, int numCities, int numVehicle, int vehicleCapacity, const std::vector<std::pair<int, int>>& demand, const std::vector<std::pair<int, int>>& coordinates, const std::string& benchmarkID, const std::vector<std::vector<double>>& distanceMatrix) {
    int chromosomeSize = numCities + numVehicle;
    ...
    for (int i = 0; i < populationSize; ++i) {
        ...
        int routeNum, feasibility, startNode, endNode, currentLoad; double travelDistance;
        std::vector<int> fixedNode(numVehicle, 0);                
        ...
        int routeNumTemporary, feasibilityTemporary, startNodeTemporary, endNodeTemporary, currentLoadTemporary; double travelDistanceTemporary;
        int iterationInitialPopulation = 0; int maxIterationInitialPopulation = 10;
        int repairingIteration = 0; int maxRepairingIteration = 3 * numVehicle;
        ...
        do {
            ...
            do {                
                ...
                sortCitiesByDemand(demanddescendingsort);
                for (int v = 0; v < numVehicle; ++v)
                {
                    servedCities[demanddescendingsort[v].first] = 1;
                }
                 
                for (int v = 0; v < numVehicle; ++v)
                {
                    fixedNode[v] = demanddescendingsort[v].first;
                }

                for (v = 0; v < numVehicle; ++v)
                {
                    routeNum = v; startNode = j;
                    population[i].route[j] = 0; servedCities[0] = 1; j = j + 1;
                    population[i].route[j] = fixedNode[v];
                    currentLoad = demand[fixedNode[v]].second;
                    travelDistance = distanceMatrix[population[i].route[j - 1]][population[i].route[j]];
                    j = j + 1;
                    //std::cout << "i= " << i << ", j= " << j << ", v= " << v << "\n\n\n" << std::endl;
                    forbiddenNumbers.clear();
                    for (int f = 0; f < numCities; ++f) {
                        if (servedCities[f] == 1) {
                            forbiddenNumbers.insert(f);
                        }
                    }
                    if (allCitiesServed(servedCities, numCities))
                    {
                        population[i].route[j] = 0;
                        forbiddenNumbers.insert(0);  
                        servedCities[0] = 1; 
                    }
                    else {
                        int currentLoadTry = 0;
                        int randomNumber = -1;  
                        do { 
                            currentLoadTry = currentLoad;
                            do {
                                randomNumber = generateRandomNumber(numCities, forbiddenNumbers);
                                currentLoad = currentLoadTry + demand[randomNumber].second;
                            } while (currentLoad > vehicleCapacity && servedCities[randomNumber] == 1);  
                            population[i].route[j] = randomNumber;
                            
                            servedCities[randomNumber] = 1;
                            travelDistance = travelDistance + distanceMatrix[population[i].route[j - 1]][population[i].route[j]];
                            forbiddenNumbers.insert(randomNumber);
                            for (const auto& node : demand) {
                                if (node.second > vehicleCapacity - currentLoad) {
                                    forbiddenNumbers.insert(node.first);
                                }
                            } 
                            ++j;
                            servedCount = countServedCities(servedCities);
                        } while (forbiddenNumbers.size() < numCities && currentLoad < vehicleCapacity);
                    }
            ...
            } while (numCities != servedCount && iterationInitialPopulation < maxIterationInitialPopulation);
                                    
            if (numCities != servedCount) {
                do {
                    int j = 0; int node; 
                    routeNum = 0, feasibility = 0, startNode = 0, endNode = 0, currentLoad = 0, travelDistance = 0;
                    std::fill(routeDetailTemporary.begin(), routeDetailTemporary.end(), std::make_tuple(0, 0, 0, 0, 0, 0));
                    routeNumTemporary = 0, feasibilityTemporary = 0, startNodeTemporary = 0, endNodeTemporary = 0, currentLoadTemporary = 0, travelDistanceTemporary = 0;
                    std::fill(temporaryRouteChecker.begin(), temporaryRouteChecker.end(), 0);
                    for (int v = 0; v < numVehicle; ++v) {
                        feasibility = std::get<1>(routeDetail[v]);                        
                        if (feasibility == 1) {                            
                            startNodeTemporary = j;
                            feasibilityTemporary = 1;
                            startNode = std::get<2>(routeDetail[v]);
                            do
                            {
                                temporaryRouteChecker[j] = population[i].route[startNode];
                                ++j;
                                ++startNode;
                                node = population[i].route[startNode];
                            } while (node != 0);
                            endNodeTemporary = j;
                            currentLoadTemporary = std::get<4>(routeDetail[v]);
                            travelDistanceTemporary = std::get<5>(routeDetail[v]);
                            routeDetailTemporary[routeNumTemporary] = std::make_tuple(routeNumTemporary, feasibilityTemporary, startNodeTemporary, endNodeTemporary, currentLoadTemporary, travelDistanceTemporary);
                            ++routeNumTemporary;                
                    ...
                    for (int v = 0; v < numVehicle; ++v)
                    {
                        fixedNode[v] = temporaryRouteChecker[std::get<2>(routeDetailTemporary[v]) + 1];
                    }
                    std::copy(temporaryRouteChecker.begin(), temporaryRouteChecker.end(), population[i].route.begin());
                    routeDetail = routeDetailTemporary;                    
                    routeNum = 0; feasibility = 0; startNode = 0; endNode = 0;
                    int routeNumRepair = 0, startNodeRepair = 0, endNodeRepair = 0;
                    if (routeDetail.empty()) {
                        std::cerr << "Error: routeDetail is empty!" << std::endl;
                        return population;
                    }

                    routeNum = 0;
                    do {
                        if (routeNum >= routeDetail.size()) { 
                            std::cerr << "Error: routeNum exceeded routeDetail size!" << std::endl;
                            return population;
                        }
                        feasibility = std::get<1>(routeDetail[routeNum]);
                        ++routeNum;
                    } while (feasibility == 1);

                    routeNumRepair = routeNum - 1;

                    if (routeNumRepair >= routeDetail.size()) { 
                        std::cerr << "Error: routeNumRepair is out of range!" << std::endl;
                        return population;
                    }
                    startNodeRepair = std::get<2>(routeDetail[routeNumRepair]);
                    endNodeRepair = std::get<3>(routeDetail[routeNumRepair]);
                    for (j = 0; j < startNodeRepair; ++j)
                    {
                        servedCities[temporaryRouteChecker[j]] = 1;
                    }
                    for (int v = 0; v < numVehicle; ++v)
                    {
                        servedCities[fixedNode[v]] = 1;
                    }
                    j = startNodeRepair; int nodeRepair = 0; 
                    for (int v = routeNumRepair; v < numVehicle; ++v)
                    {                        
                        routeNum = v, feasibility = 0, startNode = 0, endNode = 0;
                        routeDetail[v] = std::make_tuple(routeNum, feasibility, startNode, endNode, currentLoad, travelDistance);
                        do {
                            population[i].route[j] = 0; 
                            startNodeTemporary = std::get<2>(routeDetailTemporary[v]);
                            if (j == startNodeTemporary + 1) { servedCities[temporaryRouteChecker[j]] = 1; }
                            else { servedCities[temporaryRouteChecker[j]] = 0; } 
                            j = j + 1;
                            nodeRepair = population[i].route[j];
                        } while (nodeRepair != 0);
                    }
                    j = startNodeRepair;
                    ...
                    for (int v = routeNumRepair; v < numVehicle; ++v) 
                    {
                        ...
                        int availableCapacity = vehicleCapacity - currentLoad;
                        if (availableCapacity <= 0) { feasibility = 1; }
                        else { feasibility = 0; }
                        travelDistance = travelDistance + distanceMatrix[population[i].route[j - 1]][population[i].route[j]];
                        endNode = j;
                        routeDetail[v] = std::make_tuple(routeNum, feasibility, startNode, endNode, currentLoad, travelDistance);      
                    ...
                    servedCount = countServedCities(servedCities);         
                    std::cout << "\n\n\n\n\nRepairing Iteration :" << repairingIteration << "\n\n\n\n\n" << std::endl;
                    ++repairingIteration;
                } while (repairingIteration < maxRepairingIteration && numCities != servedCount);                
                servedCount = countServedCities(servedCities);
            }            
        } while (numCities != servedCount);
        for (int v = 0; v < numVehicle; ++v) {           
            ...
            individuDetail[i][v] = IndividuDetail(feas, sNode, eNode, cLoad, tDist);
        }
    }
    return population;
}
...
int main() {
    ...
    std::string path2 = std::string("d:\\FoG(CARI)=")
        + benchmarkID
